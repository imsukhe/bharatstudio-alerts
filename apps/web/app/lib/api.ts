...
export function moderateAlert(channelId: string, alertId: string, action: 'approve' | 'hold' | 'suppress' | 'replay', reason?: string): Promise<{ eventId: string; action: string; appliedAt: string }> {
  return apiFetch(`/v1/channels/${pathSegment(channelId)}/moderation/${pathSegment(alertId)}`, { method: 'POST', body: JSON.stringify({ action, ...(reason ? { reason } : {}) }) }, parseModerationResult);
}
