-- L03/L05 replay safety after concurrent task publication.
--
-- A later delivery may be published before an older delivery when task
-- enqueueing, network delivery or worker execution completes out of order.
-- The cursor is an acknowledgement checkpoint, not an eligibility filter:
-- an older row that is still unacknowledged must remain replayable or it can
-- be skipped permanently after the newer cursor is acknowledged.
