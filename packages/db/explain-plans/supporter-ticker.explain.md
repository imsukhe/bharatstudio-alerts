# supporter-ticker — EXPLAIN plan artifact (RT-12)

Function: `app_private.list_overlay_supporter_ticker(uuid, text)`
Defined at: `packages/db/migrations/0108_v1_l16_paid_votes_and_missing_widgets.sql:380` (block spans lines 380-405)
...
SELECT * FROM app_private.list_overlay_supporter_ticker('00000000-0000-0000-0000-000000000003'::uuid, 'aca8464c4445892c4627b19f1335ae66b84c87976e994d973b1c15769cf3f56d');
```

query_hash: `49c58e21fd2264836f2e3fb801c2ab4980bd3d9503f8c5ebdd5ecbd2a8efed10`

Computed by extracting the exact text from the `create or replace function app_private.list_overlay_supporter_ticker(` line through the terminating `$$;` line (inclusive) out of `packages/db/migrations/0108_v1_l16_paid_votes_and_missing_widgets.sql` as it exists at capture time, then `sha256sum`-ing that extracted text verbatim (UTF-8, including original newlines/whitespace). This is the identical extraction `packages/db/explain-plans/check-plans.mjs` re-runs against the migration file's CURRENT contents on every check; a hash mismatch means the function body changed since this artifact was captured and the plan needs re-verification.
...
Wrapper call: opaque `Function Scan on list_overlay_supporter_ticker` (security definer, never inlined). Unwrapped body: `Limit -> Sort -> Nested Loop(Index Scan using overlay_sessions_pkey on overlay_sessions, Seq Scan on payments)`. `Seq Scan on payments` is honest at this seed size (5 synthetic rows total, 2 matching) — not evidence of production-size behaviour.
...
```
Function Scan on list_overlay_supporter_ticker  (cost=0.25..10.25 rows=1000 width=72) (actual time=1.615..1.615 rows=2 loops=1)
...
```
Limit  (cost=21.35..21.36 rows=1 width=72) (actual time=0.028..0.029 rows=2 loops=1)
  Buffers: shared hit=6
  ->  Sort  (cost=21.35..21.36 rows=1 width=72) (actual time=0.028..0.028 rows=2 loops=1)
        Sort Key: payment.created_at DESC
        Sort Method: quicksort  Memory: 25kB
        Buffers: shared hit=6
        ->  Nested Loop  (cost=0.15..21.34 rows=1 width=72) (actual time=0.014..0.015 rows=2 loops=1)
              Join Filter: (session.channel_id = payment.channel_id)
              ...
              ->  Seq Scan on payments payment  (cost=0.00..13.12 rows=2 width=48) (actual time=0.003..0.003 rows=2 loops=1)
                    Filter: ((viewer_identity_id IS NOT NULL) AND (status = ANY ('{captured,partially_refunded}'::text[])))
