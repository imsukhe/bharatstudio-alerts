# mega-tip-banner — EXPLAIN plan artifact (RT-12)

Function: `app_private.list_overlay_mega_tip_banner(uuid, text)`
Defined at: `packages/db/migrations/0108_v1_l16_paid_votes_and_missing_widgets.sql:416` (block spans lines 416-438)
...
SELECT * FROM app_private.list_overlay_mega_tip_banner('00000000-0000-0000-0000-000000000003'::uuid, 'aca8464c4445892c4627b19f1335ae66b84c87976e994d973b1c15769cf3f56d');
```

query_hash: `55f529ce80277d52c39fd216d6ad656a0255626fc5a71bdf05959488c2520aab`

Computed by extracting the exact text from the `create or replace function app_private.list_overlay_mega_tip_banner(` line through the terminating `$$;` line (inclusive) out of `packages/db/migrations/0108_v1_l16_paid_votes_and_missing_widgets.sql` as it exists at capture time, then `sha256sum`-ing that extracted text verbatim (UTF-8, including original newlines/whitespace). This is the identical extraction `packages/db/explain-plans/check-plans.mjs` re-runs against the migration file's CURRENT contents on every check; a hash mismatch means the function body changed since this artifact was captured and the plan needs re-verification.
...
Wrapper call: opaque `Function Scan on list_overlay_mega_tip_banner` (security definer, never inlined). Unwrapped body: `Limit -> Sort -> Nested Loop(Nested Loop(Index Scan using overlay_sessions_pkey on overlay_sessions, Seq Scan on alert_events), Seq Scan on payments)`. Both Seq Scans are honest at this seed size (2-5 synthetic rows) — not evidence of production-size behaviour.
...
```
Function Scan on list_overlay_mega_tip_banner  (cost=0.25..10.25 rows=1000 width=48) (actual time=1.896..1.896 rows=1 loops=1)
...
```
Limit  (cost=37.68..37.68 rows=1 width=48) (actual time=0.029..0.029 rows=1 loops=1)
  Buffers: shared hit=7
  ->  Sort  (cost=37.68..37.68 rows=1 width=48) (actual time=0.028..0.029 rows=1 loops=1)
        Sort Key: event.created_at DESC
        Sort Method: quicksort  Memory: 25kB
        Buffers: shared hit=7
        ->  Nested Loop  (cost=0.15..37.67 rows=1 width=48) (actual time=0.015..0.016 rows=1 loops=1)
              Join Filter: (payment.id = event.payment_id)
              Rows Removed by Join Filter: 1
              Buffers: shared hit=4
              ->  Nested Loop  (cost=0.15..23.59 rows=1 width=56) (actual time=0.010..0.011 rows=1 loops=1)
                    ...
                    ->  Seq Scan on alert_events event  (cost=0.00..15.40 rows=1 width=72) (actual time=0.002..0.002 rows=1 loops=1)
                          Filter: ((source_type = 'payment'::text) AND (created_at >= (CURRENT_TIMESTAMP - '00:15:00'::interval)))
                          Rows Removed by Filter: 1
                          Buffers: shared hit=1
              ->  Seq Scan on payments payment  (cost=0.00..14.06 rows=1 width=24) (actual time=0.002..0.002 rows=2 loops=1)
                    Filter: ((gross_amount_paise >= 500000) AND (status = ANY ('{captured,refunded,partially_refunded}'::text[])))
