...
// for a valid session. So `null` here means "not authorised / no
// answer" and `{ heldCount: 0 }` means "authorised, and nothing is
// held". The Canvas module renders those two differently.
...
export function createSqlModeratorStatusOverlayStore(sql: Sql): ModeratorStatusOverlayStore {
  return {
    async getForOverlay(token, overlayId): Promise<ModeratorStatus | null> {
      // One column selected, because one column is all the function
      // returns. Widening this select is not possible without widening
      // migration 0136's own `returns table` signature, which
      // packages/db/tests/prf02_slice5_moderator_status.sql asserts
      // against directly.
      const rows = await sql<{ held_count: string | number }[]>`
        select held_count
