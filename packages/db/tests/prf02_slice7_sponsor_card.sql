...
values
  ('00000000-0000-4000-8000-000000005b11', '00000000-0000-4000-8000-000000000001', 'sponsor_a', 'Sponsor A', true, 1, current_timestamp, current_timestamp),
  ('00000000-0000-4000-8000-000000005b12', '00000000-0000-4000-8000-000000000002', 'sponsor_b', 'Sponsor B', true, 1, current_timestamp, current_timestamp),
  ('00000000-0000-4000-8000-000000005b13', '00000000-0000-4000-8000-000000000001', 'sponsor_c', 'Sponsor C', true, 1, current_timestamp, current_timestamp),
  ('00000000-0000-4000-8000-000000005b14', '00000000-0000-4000-8000-000000000001', 'sponsor_d', 'Sponsor D', true, 1, current_timestamp, current_timestamp)
...
values
  ...
  ('00000000-0000-4000-8000-000000005b13', '00000000-0000-4000-8000-000000000001', 'owner', current_timestamp),
  ('00000000-0000-4000-8000-000000005b14', '00000000-0000-4000-8000-000000000001', 'owner', current_timestamp)
...
values
  ('00000000-0000-4000-8000-000000005b41', '00000000-0000-4000-8000-000000005b11', 'prf02s7-sponsor-a-fingerprint', current_timestamp + interval '1 hour', current_timestamp),
  ('00000000-0000-4000-8000-000000005b42', '00000000-0000-4000-8000-000000005b12', 'prf02s7-sponsor-b-fingerprint', current_timestamp + interval '1 hour', current_timestamp),
  ('00000000-0000-4000-8000-000000005b43', '00000000-0000-4000-8000-000000005b13', 'prf02s7-sponsor-c-fingerprint', current_timestamp + interval '1 hour', current_timestamp),
  ('00000000-0000-4000-8000-000000005b44', '00000000-0000-4000-8000-000000005b11', 'prf02s7-sponsor-expired-fingerprint', current_timestamp - interval '1 minute', current_timestamp - interval '2 hours');

insert into overlay_sessions (id, channel_id, token_fingerprint, expires_at, created_at, revoked_at)
values
  ('00000000-0000-4000-8000-000000005b45', '00000000-0000-4000-8000-000000005b11', 'prf02s7-sponsor-revoked-fingerprint', current_timestamp + interval '1 hour', current_timestamp, current_timestamp);
...
begin
  ...
  select app_private.upsert_sponsor_card(
    '00000000-0000-4000-8000-000000005b11'::uuid, 'Acme Energy Drinks',
...
begin
  ...
  begin
    perform app_private.upsert_sponsor_card(
      '00000000-0000-4000-8000-000000005b11'::uuid, 'Hijacked Sponsor',
      null, null, null, true, null, null
    );
...
begin
  select sponsor_name into current_name from public.sponsor_cards where channel_id = '00000000-0000-4000-8000-000000005b11';
...
begin
  perform set_config('app.user_id', '00000000-0000-4000-8000-000000000001', true);
  begin
    perform app_private.upsert_sponsor_card('00000000-0000-4000-8000-000000005b11'::uuid, '', null, null, null, true, null, null);
  ...
  begin
    perform app_private.upsert_sponsor_card('00000000-0000-4000-8000-000000005b11'::uuid, repeat('x', 121), null, null, null, true, null, null);
...
begin
  perform set_config('app.user_id', '00000000-0000-4000-8000-000000000001', true);
  select app_private.upsert_sponsor_card('00000000-0000-4000-8000-000000005b11'::uuid, 'Acme Energy Drinks', null, null, null, true, null, null) into first_id;
  select app_private.upsert_sponsor_card('00000000-0000-4000-8000-000000005b11'::uuid, 'Acme Energy Drinks V2', null, null, null, true, null, null) into second_id;
  if first_id <> second_id then raise exception 'SP11.4: two writes to the same channel produced two different ids'; end if;

  select count(*) into row_count from public.sponsor_cards where channel_id = '00000000-0000-4000-8000-000000005b11';
...
begin
  ...
  select logo_storage_key into seen_key from public.sponsor_cards where channel_id = '00000000-0000-4000-8000-000000005b11';
  expected_key := '00000000-0000-4000-8000-000000005b11/' || expected_hash;
...
begin
  begin
    execute 'update public.sponsor_cards set logo_storage_key = ''hijacked'' where channel_id = ''00000000-0000-4000-8000-000000005b11''';
...
begin
  ...
  perform app_private.upsert_sponsor_card(
    '00000000-0000-4000-8000-000000005b13'::uuid, 'Disabled Sponsor',
...
begin
  ...
  perform app_private.upsert_sponsor_card(
    '00000000-0000-4000-8000-000000005b12'::uuid, 'Scheduled Sponsor',
...
begin
  select * into seen from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b41'::uuid, 'prf02s7-sponsor-a-fingerprint'
  );
  ...
  if seen.logo_storage_key is distinct from ('00000000-0000-4000-8000-000000005b11/' || repeat('ab', 32)) then
...
create temporary table prf02s7_sponsor_returned_shape as
  select * from app_private.list_overlay_sponsor_card('00000000-0000-4000-8000-000000005b41'::uuid, 'prf02s7-sponsor-a-fingerprint');
...
begin
  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b43'::uuid, 'prf02s7-sponsor-c-fingerprint'
  );
...
begin
  ...
  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b42'::uuid, 'prf02s7-sponsor-b-fingerprint'
  );
...
begin
  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b41'::uuid, 'wrong-fingerprint-entirely'
  );
  if row_count <> 0 then raise exception 'SP11.17: a wrong fingerprint must return zero rows, got %', row_count; end if;

  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b44'::uuid, 'prf02s7-sponsor-expired-fingerprint'
  );
  if row_count <> 0 then raise exception 'SP11.17: an expired session must return zero rows, got %', row_count; end if;

  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b45'::uuid, 'prf02s7-sponsor-revoked-fingerprint'
  );
  ...
  insert into overlay_sessions (id, channel_id, token_fingerprint, expires_at, created_at)
  values ('00000000-0000-4000-8000-000000005b46', '00000000-0000-4000-8000-000000005b14', 'prf02s7-sponsor-d-fingerprint', current_timestamp + interval '1 hour', current_timestamp);
  select count(*) into row_count from app_private.list_overlay_sponsor_card(
    '00000000-0000-4000-8000-000000005b46'::uuid, 'prf02s7-sponsor-d-fingerprint'
  );
