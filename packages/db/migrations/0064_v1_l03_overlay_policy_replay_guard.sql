-- Preserve the L03 replay contract while applying the new dispatch policy:
-- Last-Event-ID is an acknowledgement checkpoint, never a deletion/filter
-- boundary. Older unacknowledged rows must remain replayable after a newer
-- row was published first.
...
as $$
  select to_char(delivery.created_at at time zone 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') || '|' || delivery.id::text,
         event.id, 'alert.ready', event.trace_id, delivery.created_at,
         event.payload || jsonb_build_object('deliveryId', delivery.id,
           'queueId', delivery.queue_id, 'bindingId', delivery.binding_id,
