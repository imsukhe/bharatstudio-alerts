...
function unavailable(reply: { code: (status: number) => { send: (body: unknown) => unknown } }, traceId: string) {
  return reply.code(503).send({ schemaVersion: 'v1', errorCode: 'overlay_audio_unavailable', message: 'Overlay audio is temporarily unavailable', traceId, retryable: true });
}
...
export async function registerOverlayAudioRoutes(app: FastifyInstance, store?: OverlayAudioStore): Promise<void> {
  app.get<{ Params: { overlayId: string; artifactId: string }; Headers: { authorization?: string } }>('/v1/overlay-audio/:overlayId/:artifactId', {
    ...
    if (!token) return reply.code(401).send({ schemaVersion: 'v1', errorCode: 'overlay_unauthorized', message: 'Overlay audio is not available', traceId: request.id });
    ...
    try {
      ...
      logSafeError(request, 'overlay_audio_read_failed', error);
