...
async function buildOverlayApp(store?: Partial<SponsorCardOverlayStore>) {
  ...
  await registerMasterCanvasRoutes(
    ...
    // slice-7 merge: positions 10 and 11 are overlayMediaQueue and
    // overlaySafeSoundboard, so the sponsor store is 12th. Positional, so
    // the gap is written out rather than assumed.
    undefined, undefined,
