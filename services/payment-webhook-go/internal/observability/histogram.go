...
// (Metrics.tipOrderDuration, Metrics.webhookAckDuration) is only ever
// touched while the owning Metrics' own mutex is held (see metrics.go's
// Observe/WritePrometheus) -- the same discipline every other Metrics field
// already follows. A Histogram used outside that discipline needs its own
