...
export async function registerChannelRoutes(app: FastifyInstance, sessions?: SessionStore, store?: ChannelStore): Promise<void> {
  ...
  app.patch<{ Params: { channelId: string }; Body: { displayName?: string; acceptingTips?: boolean } }>('/v1/channels/:channelId', { preHandler: auth, schema: { params: channelParams, body: { type: 'object', additionalProperties: false, minProperties: 1, properties: { displayName: { type: 'string', minLength: 1, maxLength: 120 }, acceptingTips: { type: 'boolean' } } } } }, async (request, reply) => {
    if (!store || !request.auth) return unavailable(reply, request.id);
  ...
  app.patch<{ Params: { channelId: string }; Headers: { 'if-match-version': string }; Body: { values: Record<string, unknown> } }>('/v1/channels/:channelId/config', { preHandler: auth, schema: { params: channelParams, headers: { type: 'object', required: ['if-match-version'], properties: { 'if-match-version': { type: 'string', pattern: '^[1-9][0-9]*$' } } }, body: { type: 'object', additionalProperties: false, required: ['values'], properties: { values: channelConfigValueSchema } } } }, async (request, reply) => {
    if (!store || !request.auth) return unavailable(reply, request.id);
  ...
  app.post<{ Params: { channelId: string }; Body: { name: string } }>('/v1/channels/:channelId/queues', { preHandler: auth, schema: { params: channelParams, body: { type: 'object', additionalProperties: false, required: ['name'], properties: { name: { type: 'string', minLength: 1, maxLength: 80 } } } } }, async (request, reply) => {
    if (!store || !request.auth) return unavailable(reply, request.id);
    try {
