-- L05: project the immutable channel configuration used when the alert was accepted.
--
-- The browser must never read the channel's current settings for an already
-- accepted delivery. Joining by config_snapshot_version keeps replay stable
-- across edits, reconnects and multiple queues. Only validated channel config
-- JSON is exposed; unknown delivery override keys remain client-ignored.
