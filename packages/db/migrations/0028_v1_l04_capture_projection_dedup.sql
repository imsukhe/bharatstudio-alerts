-- L04 business-effect deduplication for distinct Razorpay delivery IDs.
-- Razorpay documents order.paid and payment.captured as event types emitted
-- for the same captured payment. The provider-event ledger keeps both IDs,
-- while the payment-linked alert/outbox projection is created only once.
